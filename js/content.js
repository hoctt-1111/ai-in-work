...
| # | Nội dung | Thời lượng |
| --- | --- | --- |
| 0 | <a class="agenda-link" data-goto="1">📊 Opening & Warmup — Thực trạng từ Survey</a> | 3 phút |
| 1 | <a class="agenda-link" data-goto="2">🌏 Bối cảnh: Vai trò BrSE/Comtor & AI là "must-have"</a> | 3 phút |
| 2 | <a class="agenda-link" data-goto="3">🔒 Nguyên tắc Vàng: Bảo mật & Compliance</a> | 7 phút |
| 3 | <a class="agenda-link" data-goto="4">🎯 Kỹ thuật Prompt — Framework CRAFT</a> | 10 phút |
| 4 | <a class="agenda-link" data-goto="5">✉️ Case Study 1 — Giao tiếp khách hàng bằng tiếng Nhật</a> | 25 phút |
| 5 | <a class="agenda-link" data-goto="6">📑 Case Study 2 — Đọc hiểu & Viết Specification</a> | 30 phút |
| 6 | <a class="agenda-link" data-goto="7">📋 Case Study 3 — Quản lý dự án với AI</a> | 20 phút |
| 7 | <a class="agenda-link" data-goto="8">💡 Case Study 4 — Ứng dụng AI sáng tạo khác</a> | 15 phút |
| 8 | <a class="agenda-link" data-goto="9">✅ Quy trình Hậu kiểm — AI output ≠ Final output</a> | 5 phút |
| 9 | <a class="agenda-link" data-goto="10">🚀 Closing — Lộ trình áp dụng & Tài nguyên</a> | 2 phút |
...
| 🔴 Đặc biệt nhạy cảm | Source code khách, data thật, credentials | Premium tool + Data ở local (Cần xác nhận trước khi dùng với KH) |

---

## GIẢI PHÁP 1: Kỹ thuật Anonymize

**Mục đích:** Loại bỏ thông tin định danh (PII) khỏi tài liệu → dùng bất kỳ AI nào an toàn, không lo rò rỉ.

**Cách thực hiện:** Thay tên công ty/dự án bằng tên giả, xóa credentials/API key, thay tên người bằng chức danh.

*Ví dụ:*
- ❌ \`Mitsui Agro API key: sk-abc123 - Tanaka-buchō\`
- ✅ \`Company A - API endpoint - 部長 level\`

**Use-case phù hợp:** Email, spec, meeting notes, grammar check — Các template, thuật ngữ chung, khi context cụ thể không quá quan trọng.

---

## GIẢI PHÁP 2: NotebookLM + Gemini Pro

**Mục đích:** Upload nhiều tài liệu dự án → AI đọc toàn bộ context → Q&A, rút glossary, dịch spec chính xác theo context dự án.

**Cách thực hiện:** Vào NotebookLM → upload tài liệu làm sources → hỏi AI bất kỳ câu nào liên quan.

**Use-case phù hợp:** Project nhiều tài liệu cần access thường xuyên, onboarding member mới, share context với cả team.

---

## GIẢI PHÁP 3: GitHub Copilot / Claude Pro

**Mục đích:** Tool có data security guarantee + đọc file local trực tiếp → không cần anonymize với data nhạy cảm hơn.

**Cách thực hiện:** Mở folder project local trong IDE / chat → attach file spec, glossary, QA list → chat với AI có đầy đủ context.

> 💡 Có thể kết hợp MCP Server để access thêm Confluence, Google Drive, Figma...

**Use-case phù hợp:** Data nhạy cảm cần giữ nguyên, cần edit file trực tiếp, tích hợp vào dev workflow hàng ngày.

---

## So sánh 3 giải pháp

| Tiêu chí | 1. Anonymize | 2. NotebookLM | 3. Copilot / Claude Pro |  Note |
|---|---|---|---|---|
| **Giá** | Miễn phí | Có phí | Có phí | Công ty đã có sẵn Gemini/NotebookLM PRO |
| **Data policy** | User responsibility | Google Workspace | Guarantee no training | — |
| **Cross-docs** | ❌ Không | ✅ 10+ files upload | ✅ File local / clipboard | NotebookLM cho spec dài, Copilot cho dev |
| **Team sharing** | ❌ Khó sync | ✅ Share notebook link | ✅ Cần upload/publish output trước khi share | — |
| **Tốc độ phản hồi** | ⚡ Nhanh | 🟡 Trung bình | ⚡ Nhanh | — |
| **Phù hợp nhất** | Các template, thuật ngữ chung | Spec dài + dự án nhiều tài liệu + team sharing | Data nhạy cảm + dev workflow | **Kết hợp cả 3 tùy vào bài toán cụ thể** |
...
### Bước 2A: Input glossary vào NotebookLM để dịch spec
...
### Bước 2B: Input glossary vào ChatGPT
...
- Có thể **share bảng thuật ngữ nguyên ngành cho cả team** — đảm bảo consistency thuật ngữ giữa các member
...
**Update 202604:** 
- GenAI đã có khả năng **tự đọc file audio/video** → có thể bỏ qua bước transcript thủ công, upload trực tiếp vào GenAI để tóm tắt + tạo 議事録.
- Sun* vừa release tool mới có thể dịch real-time 2 chiều, tự động tạo meeting minutes và đảm bảo security tuyệt đối [MEET PLUS](https://meet-plus.sun-asterisk.ai/dashboard) → Highly recommend. 
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | DBテーブル設計 | 設計 | 0.75 | 0 | 0 | 0 | 0.75 | — | ⚠️ 棚卸中の在庫凍結フラグのカラム設計を事前確認 |
| 2 | 棚卸開始API（在庫凍結） | 実装 | 1.5 | 0 | 0 | 0 | 1.5 | #1 | ⚠️ 複数倉庫同時棚卸の可否をKHに確認が必要 |
| 3 | 実棚入力API | 実装 | 1.25 | 0 | 0 | 0 | 1.25 | #1 | — |
| 4 | 差異計算・承認API | 実装 | 1.75 | 0 | 0 | 0 | 1.75 | #3 | ⚠️ 承認者が不在の場合の代理承認フローを確認 |
| 5 | 棚卸開始画面（SCR-STK-001） | 実装 | 0 | 0.75 | 0 | 0 | 0.75 | #2 | — |
| 6 | 実棚入力画面（SCR-STK-002） | 実装 | 0 | 1.25 | 0 | 0 | 1.25 | #3 | ⚠️ オフライン入力（ネット不安定な倉庫）の対応要否を確認 |
| 7 | 差異確認画面（SCR-STK-003） | 実装 | 0 | 1.25 | 0 | 0 | 1.25 | #4 | ⚠️ ハイライト閾値（±5%, ±10%）は変更可能にするか確認 |
| 8 | コードレビュー（BE + FE） | レビュー | 0.75 | 0.75 | 0 | 0 | 1.5 | #7 | — |
| 9 | テスト計画・ケース作成 | テスト | 0 | 0 | 1.25 | 0 | 1.25 | #7 | ⚠️ 差異率10%超の承認フローを含むE2Eシナリオが必要 |
| 10 | 結合テスト実施 | テスト | 0 | 0 | 1.5 | 0 | 1.5 | #9 | — |
| 11 | DBマイグレーション・環境設定 | 構築 | 0 | 0 | 0 | 0.75 | 0.75 | #1 | ⚠️ Staging環境でのマイグレーション手順のレビューが必要 |
...
🛠️ **Tools:** GitHub Copilot (Agent mode) · Claude.ai (attach file) · Không cần framework

**Quy trình:**

| Step | Người thực hiện | Công cụ | Lưu ý |
|---|---|---|---|
| 1. Chuẩn bị feature list + danh sách màn hình | BrSE | — | Mô tả UI style càng chi tiết, output AI càng sát |
| 2. Tạo layout tổng thể + navigation | BrSE + AI | Copilot Agent / Claude | Tạo xương sườn trước rồi mới điền từng màn |
| 3. Thêm từng màn hình + dummy data | BrSE + AI | Copilot Agent / Claude | Dummy data dùng tên JP, format ngày JP (YYYY/MM/DD) |
| 4. Review & chỉnh sửa | BrSE | Browser | Kiểm tra navigation, responsive, dummy data |
| 5. Gửi cho khách | BrSE | Email | 1 file HTML duy nhất — không cần deploy |
...
## 🧮 CASE B: Tự tạo công cụ cá nhân

🛠️ **Tools:** GitHub Copilot (Agent mode) · Netlify/GitHub Pages (deploy)
...
- Dark/Light mode
- Keyboard shortcut
- Deploy miễn phí trên Netlify/GitHub Pages

**Quy trình:**

| Step | Người thực hiện | Công cụ | Lưu ý |
|---|---|---|---|
| 1. Mô tả ý tưởng + yêu cầu | Cá nhân | — | Viết bằng tiếng Việt tự nhiên, liệt kê hết features muốn có |
| 2. AI sinh spec chi tiết | Cá nhân + AI | Copilot Agent / Claude | Spec làm "hợp đồng" với AI trong các bước sau |
| 3. AI sinh code từ spec | Cá nhân + AI | Copilot Agent / Claude | Bắt đầu từ 1 file HTML đơn giản, tăng dần |
| 4. Test → feedback → AI sửa | Cá nhân | Browser | Describe bug cụ thể: "Button X không làm gì khi click" |
| 5. Deploy | Cá nhân | Netlify / GitHub Pages | Free, 5 phút setup |
...
## 🎨 CASE C: Sinh Presentation/Sharing Page

🛠️ **Tools:** GitHub Copilot (Agent mode + file edit) · GitHub Pages (deploy)
...
| Step | Người thực hiện | Công cụ | Lưu ý |
|---|---|---|---|
| 1. Viết outline các section | Cá nhân | — | Bullet points đơn giản là đủ |
| 2. AI sinh content từng section | Cá nhân + AI | Copilot Agent / Claude | Iterate từng section, không làm tất cả 1 lần |
| 3. AI tạo website HTML | Cá nhân + AI | Copilot Agent / Claude | Yêu cầu: sidebar nav, dark/light mode, responsive |
| 4. Chỉnh nội dung + thêm demo | Cá nhân + AI | Copilot Agent / Claude | Copilot giúp edit từng đoạn markdown nhanh hơn |
| 5. Deploy hoặc gửi file | Cá nhân | GitHub Pages / Email | GitHub Pages free, hoặc gửi thẳng file HTML |
